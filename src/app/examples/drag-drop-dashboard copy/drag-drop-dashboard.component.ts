...
@Component({
  ...
  styles: [
    `
      ...
      /* Ensure the main vertical split respects the computed height */
    ...
    <div class="dashboard-container">
      ...
      <div class="dashboard-body">
        <div class="dashboard-content">
          ...
          @if (rows().length === 0) {
            <div class="empty-row" (click)="addRow()">🎯 Click to create your first dashboard row</div>
          } @else {
            <as-split
              direction="vertical"
              unit="pixel"
              [variableHeight]="true"
              [gutterSize]="8"
              [style.height.px]="totalDashboardHeight()"
                ...
                <as-split-area [size]="row.height" [minSize]="row.minHeight" [maxSize]="row.maxHeight">
                  <div class="dashboard-row" style="height: 100%; position: relative;">
                    ...
                    @if (dragState().isDragging) {
                      <div
                        class="drop-zone drop-zone-row drop-zone-new-row-above"
                      ...
                      <div
                        class="drop-zone drop-zone-row drop-zone-new-row-below"
                    ...
                    @if (row.panels.length === 0) {
                      <div class="empty-row" (click)="addPanelToRow(row.id)">➕ Add panel to this row</div>
                    } @else {
                      <as-split direction="horizontal" unit="percent" [gutterSize]="8" style="height: 100%">
                        <!-- Custom gutter template for horizontal splits -->
                        <ng-template asSplitGutter let-gutterNum="gutterNum" let-isDragged="isDragged">
                          <div
                            class="custom-gutter horizontal"
                            [class.dragging]="isDragged"
                            [class.panel-dragging]="dragState().isDragging"
                            [class.drop-active]="
                              dragState().dropZoneActive === 'gutter-horizontal-' + gutterNum + '-' + row.id
                            "
                            (dragover)="
                              dragState().isDragging
                                ? onDragOver($event, 'gutter-horizontal-' + gutterNum + '-' + row.id)
                                : null
                            "
                            (dragleave)="dragState().isDragging ? onDragLeave($event) : null"
                            (drop)="
                              dragState().isDragging ? onDropOnGutter($event, 'horizontal', gutterNum, row.id) : null
                            "
                          >
                            <div class="gutter-handle"></div>
                            @if (
                        ...
                        @for (panel of row.panels; track panel.id; let panelIndex = $index) {
                          <as-split-area [size]="panel.size" [minSize]="panel.minSize" [maxSize]="panel.maxSize">
                            ...
                            >
                              ...
                              @if (
                                dragState().isDragging &&
                                (dragState().draggedPanel?.id !== panel.id || row.panels.length === 1)
                              ) {
                                <div
                                  class="drop-zone drop-zone-column drop-zone-new-column-left"
                                ...
                                <div
                                  class="drop-zone drop-zone-column drop-zone-new-column-right"
                                      ...
                                      <div class="chart-container">
                                        <svg class="chart-line" viewBox="0 0 400 200">
                                          ...
                                          <path
                                            ...
                                            [attr.fill]="'url(#gradient' + panel.id + ')'"
                                          />
                                          <path
...
export class DragDropDashboardComponent extends AComponent implements OnInit {
  ...
  // Computed total height for the dashboard
  totalDashboardHeight = computed(() => {
    const currentRows = this.rows()
    if (currentRows.length === 0) return 400 // Minimum height when no rows

    // Calculate total height: sum of all row heights + gutter sizes
    const totalRowHeight = currentRows.reduce((sum: number, row: DashboardRow) => sum + row.height, 0)
    const gutterHeight = (currentRows.length - 1) * 8 // 8px per gutter
    const padding = 40 // 20px padding top + bottom

    // Ensure minimum height and proper scaling
    const computedHeight = totalRowHeight + gutterHeight + padding
    return Math.max(400, computedHeight) // Always at least 400px
  })
        ...
        if (draggedFromRow === targetRowId) {
          ...
          if (sourceRow && sourceRow.panels.length === 1) {
            ...
            const duplicatedPanel: Panel = {
              ...
              size: 50, // Split the space equally
            }
